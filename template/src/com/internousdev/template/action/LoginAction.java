...
public class LoginAction extends ActionSupport implements SessionAware {
	...
	public String execute(){
		...
		if(((LoginDTO) session.get("loginUser")).getLoginFlg()){
			 ...
			 * ④前もって(（）でくくって)キャスト(LoginDTO)をつけて型変換してしまおう
	...
	//下記Map型変数(session)の値を外へ(jspファイルへ)受け渡す時(getterを使う時)には、
	...
	@Override //implements(実装元)のSessionAwareクラスの中にこのsetterが書かれているため、コード上使ってなくても記述が必要
